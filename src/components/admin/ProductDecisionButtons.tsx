...
import { decideProduct } from "@/actions/admin";

export default function ProductDecisionButtons({ productId }: { productId: string }) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-2">
        <button
          disabled={pending}
          onClick={() => startTransition(async () => { await decideProduct(productId, true); })}
      ...
      {showReject && (
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => startTransition(async () => { await decideProduct(productId, false, reason); })}
